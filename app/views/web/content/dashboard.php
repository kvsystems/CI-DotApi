<div class="alert alert-success center-block" role="alert" style="width: 98%">
    <i class="fa fa-warning"></i> <span>&nbsp;&nbsp;Добро пожаловать в систему, приятной работы с заданиями через REST-API</span>
</div>
<style>
    .single-icon > i {
        font-size: 30px;
...
<script>
    ...
    function countTasks() {
        $.get("http://ci-dot.kv-sys.ru/api/v1/count", function(data) {
            var page = Math.ceil(data.count / <?=$perPage?>);
            $('#response').html('');
            $(".pagination").empty();
    ...
    function processPage(page)  {
        $.get("http://ci-dot.kv-sys.ru/api/v1/page/" + page, function(data) {
            $('#response').html('');
    ...
    function viewTask(taskId) {
        $.get("http://ci-dot.kv-sys.ru/api/v1/task/" + taskId, function(data) {
             $('#response').html('');
                $('#titleTask').val(data.task.taskName);
                $('#dateTask').val(data.task.taskDate);
                $('#authorTask').val(data.task.authorName);
                $('#statusTask').val(data.task.statusValue);
                $('#descTask').val(data.task.taskDescription);
    ...
    $('#searchButton').click(function () {
        ...
        $.get("http://ci-dot.kv-sys.ru/api/v1/search/" + encodeURIComponent(search), function(data) {
            $('#response').html('');
            $('#pag').hide();
            $("#responsableTable tbody > tr").remove();
            $.each(data.tasks,function(index,value) {
